...
namespace JumpQ_Task
{
    public partial class Form1 : Form
    {
       ...
       // private static string appdataroamingfolder = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);

        SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\v11.0;AttachDbFilename=|DataDirectory|\Inventory.mdf;Integrated Security=True");       

       // SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\\v11.0;AttachDbFilename=" + appdataroamingfolder + "\\Inventory.mdf;"+"Integrated Security=True;Connect Timeout=30");       
       ...
       // sql_con = new SQLiteConnection(a, true);
        ...
        private void btnBkup_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow item in InventoryTable.Rows)
            {
                String insertInvQuery = @"INSERT into Inventory(description,price,ordercost,upc,desc2,qty) VALUES(@desc1, @pr1, @order1, @upc1, @desc21, @qty1)";

                    //'"// + item[0].ToString() +"','" + item[1].ToString() + "', '" + item[2].ToString() + "','" + item[3].ToString() + "','"+@item[4].ToString() +"', '" + item[5].ToString//() + "')";
                SqlCommand Cmd = new SqlCommand(insertInvQuery, con);
               // SQLiteCommand Cmd = con.CreateCommand();
                Cmd.CommandType = CommandType.Text;
                Cmd.CommandText = insertInvQuery;
                Cmd.Parameters.Add("@desc1", SqlDbType.VarChar, 100).Value = item[0].ToString();
                Cmd.Parameters.Add("@pr1", SqlDbType.VarChar, 100).Value = item[1].ToString();
                Cmd.Parameters.Add("@order1", SqlDbType.VarChar, 100).Value = item[2].ToString();
                Cmd.Parameters.Add("@upc1", SqlDbType.VarChar, 100).Value = item[3].ToString();
                Cmd.Parameters.Add("@desc21", SqlDbType.VarChar, 100).Value = item[4].ToString();
                Cmd.Parameters.Add("@qty1", SqlDbType.VarChar, 100).Value = item[5].ToString();
                //SqlParameter sqp = Cmd.Parameters.AddWithValue("@desc", item[0].ToString());
               // SqlParameter sqp1 = Cmd.Parameters.AddWithValue("@pr", item[1].ToString());
               // SqlParameter sqp2 = Cmd.Parameters.AddWithValue("@order", item[2].ToString());
               // SqlParameter sqp3 = Cmd.Parameters.AddWithValue("@upc", item[3].ToString());
               // SqlParameter sqp4 = Cmd.Parameters.AddWithValue("@desc2", item[4].ToString());
               // SqlParameter sqp5 = Cmd.Parameters.AddWithValue("@qty", item[5].ToString());
               // Cmd.ExecuteNonQuery();
                if (Cmd.ExecuteNonQuery() == 1)
                {
                    count2++;
                    bkup.Text = count2++.ToString() + " Items";
                }
            }
